    ...
    <div class="mx-auto py-4 md:pl-4 shadow-md bg-gray-100 rounded-md">
        ...
        <div class="px-4 md:px-0 md:pr-4 mt-2.5">
            ...
            <div class="mt-2">
                ...
                <div v-if="details" class="text-sm text-justify leading-tight" ><span class="font-semibold">Features: </span>
                    <p v-html="details ? details : ''"></p>
        ...
        <div v-if="price" class="px-4 md:px-0 mt-2 flex flex-row gap-2">
            <p>Price:</p>
            <p class="text-orange-600 text-xs line-through align-super">was ${{ (price / (1 - (percentOff/100))).toFixed(2) }}</p>
            <p class="text-green-600 text-sm font-semibold">Now <span class="underline text-lg">${{ price ?? '' }}</span></p>
        </div>
